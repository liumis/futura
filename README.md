...
3. Deploy command: `php artisan migrate --force`
4. Environment variables:
   - `APP_KEY` — `php artisan key:generate --show`
   - `APP_ENV=production`
   - `APP_DEBUG=false`
   - `APP_URL` — the Cloud app URL (https)
   - `SESSION_SECURE_COOKIE=true`
   - `INTERNAL_PANEL_DOMAIN` / `CUSTOMER_PANEL_DOMAIN` — if using separate admin and customer hosts
